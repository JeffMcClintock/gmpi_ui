...
// Blend one premultiplied pixel: straight linear source-over. EVERYTHING in
// this renderer — text included — composites in linear premultiplied space,
// with one encode at endDraw. Glyph weight parity with Direct2D is handled
// where DirectWrite itself handles it, on the coverage values (the coverage
// warp in CpuTextEngine), so no transfer function appears in any blend.
inline void blendPixel(float* d, const float* sc, float c)
{
...
class BitmapPixels final : public drawing::api::IBitmapPixels
{
    ...
    //
    // Deliberately PER-LOCK, freed at unlock. It was briefly moved to the
    // Bitmap to spare endDraw's once-per-frame quantization lock the 6.4 MB
    // alloc+zero — but endDraw now quantizes in place without locking at all,
    // so a persistent buffer would only tax locked images (+50% of the fp16
    // surface, retained for the bitmap's life) and quietly change write-only
    // lock semantics: sparse writers like PanelGroupGui's exploded-skin fill
    // rely on the zero-fill below leaving untouched bytes transparent.
