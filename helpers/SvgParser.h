...
// macOS declares `Point`, `Rect` and `Size` in the GLOBAL namespace, via
...
// nearest namespace enclosing both — the global one. So on a Mac each of those
// unqualified names sees two candidates and the file stops compiling, while
// Windows and Linux build it happily.
//
// Naming them HERE fixes every use at once: unqualified lookup finds these
// aliases in the enclosing namespace and stops before it ever reaches global
// scope. The alternative — qualifying several dozen individual uses — leaves
// the next unqualified one to break the Mac build again.
using Point = gmpi::drawing::Point;
using Rect  = gmpi::drawing::Rect;
using Size  = gmpi::drawing::Size;
